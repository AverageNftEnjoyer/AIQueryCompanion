...
export type AlignedRowKind = "unchanged" | "addition" | "deletion" | "modification";

export interface AlignedSideCell {
  /** The text to render on this side ("" for padded blanks). */
  text: string;
  /** Original file line number on this side, if any (used for jump/flash). */
  lineNumber?: number;
  /** 1-based visual row index; provided for stable numbering in the viewer. */
  visualIndex: number;
}
...
/**
 * Build aligned row pairs so both panes render the same number of rows.
 * - addition:   right gets content (green), left gets "" (green)
 * - deletion:   left gets content (red),   right gets "" (red)
 * - modification (adjacent del+add): yellow on both with their texts
 * - unchanged:  both sides show the same text (no color)
 */
export function buildAlignedRows(comp: ComparisonResult): AlignedRow[] {
  ...
  for (let i = 0; i < d.length; i++) {
    ...
    if (cur.type === "unchanged") {
      ...
      rows.push({
        kind: "unchanged",
        old: { text: cur.content, lineNumber: cur.oldLineNumber, visualIndex: visOld },
        new: { text: cur.content, lineNumber: cur.newLineNumber, visualIndex: visNew },
      });
      continue;
    }
    ...
    if (cur.type === "deletion") {
      const nxt = d[i + 1];
      if (nxt && nxt.type === "addition") {
        // modification pair
        visOld++; visNew++;
        rows.push({
          kind: "modification",
          old: { text: cur.content, lineNumber: cur.oldLineNumber, visualIndex: visOld },
          new: { text: nxt.content, lineNumber: nxt.newLineNumber, visualIndex: visNew },
        });
        i++; // consume the addition
      } else {
        // pure deletion (pad right with blank)
        visOld++; visNew++;
        rows.push({
          kind: "deletion",
          old: { text: cur.content, lineNumber: cur.oldLineNumber, visualIndex: visOld },
          new: { text: "", visualIndex: visNew },
        });
      }
      continue;
    }

    if (cur.type === "addition") {
      const prev = d[i - 1];
      if (prev && prev.type === "deletion") {
        // handled as modification in deletion branch
        continue;
      }
      // pure addition (pad left with blank)
      visOld++; visNew++;
      rows.push({
        kind: "addition",
        old: { text: "", visualIndex: visOld },
        new: { text: cur.content, lineNumber: cur.newLineNumber, visualIndex: visNew },
      });
      continue;
    }
